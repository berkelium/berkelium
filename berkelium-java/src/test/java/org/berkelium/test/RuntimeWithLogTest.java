...
public class RuntimeWithLogTest extends AbstractRuntimeTest {
	...
	public void testRuntimeWithLogDispose() {
		...
		inOrder.verify(log).log(runtime, LogSource.Lib, LogType.Debug, clazz, name, debug);
		inOrder.verify(log).log(runtime, LogSource.Lib, LogType.Info, clazz, name, info);
		inOrder.verify(log).log(runtime, LogSource.Lib, LogType.Warn, clazz, name, warn);
		inOrder.verify(log).log(runtime, LogSource.Lib, LogType.Error, clazz, name, error);
	}
}
